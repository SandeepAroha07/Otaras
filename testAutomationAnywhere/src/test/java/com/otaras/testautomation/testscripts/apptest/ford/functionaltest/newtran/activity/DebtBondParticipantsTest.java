...
public class DebtBondParticipantsTest {
    	...
    	static String testDataName="debt_bond_participants";
    	
	    @Factory (dataProvider="dataProviderMethod")
	    public DebtBondParticipantsTest(Map<Object, Object> map){
	    	this.map=map;
	    ...
	    public void testCase() throws InterruptedException, Exception{
	        ...
	        try{
				  ...
				  pages.CommonBusinessComponentPage.LoginToApplication();
					  ...
					  //driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/deals/"+sObjectID+"/participants");
					  
					  	if (sActivity.equalsIgnoreCase(OR.DP_TranName_BankLoan))
							 driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/loan/"+sObjectID+"/participants");
					  	else if (sActivity.equalsIgnoreCase(OR.DP_TranName_BondIssue))
							 driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/deals/"+sObjectID+"/participants");
					  	else if (sTranName.equalsIgnoreCase("derivative"))
							 driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/derivative/"+sObjectID+"/participants");
					  	else if (sTranName.equalsIgnoreCase("ma-rfp"))
							 driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/marfp/"+sObjectID+"/participants");
					  	else if (sTranName.equalsIgnoreCase("other activity"))
							 driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/others/"+sObjectID+"/participants");
					  
					   
					  pages.ActivityDashboardPage.WaitForPage(pages.DebtParticipantsPage);
...
//					  pages.DebtParticipantsPage.enterDebtParticipants_BondIssue(map,objSubTran);
					  ...
					  ResultLog.ReportTestEvent("INFO", "Participant URL: "+TestDataRec.TESTDATALOGINMAP.get("url")+"/deals/"+sObjectID+"/participants");
					  if (sActivity.equalsIgnoreCase(OR.DP_TranName_BondIssue))
						  pages.DebtParticipantsPage.enterDebtParticipants_BondIssue(map,recordsubsetTestData);
					  else if (sTranName.equalsIgnoreCase("derivative"))
						  pages.DebtParticipantsPage.enterDebtParticipants_Derivatives(map,recordsubsetTestData);
					  else if (sTranName.equalsIgnoreCase("other activity"))
						  pages.DebtParticipantsPage.enterDebtParticipants_Others(map,recordsubsetTestData);
					  
					  pages.DebtParticipantsPage.enterDebtParticipants_Participants(map,recordsubsetTestData);
					  pages.DebtParticipantsPage.verifyDataEntered(map);
...
//					  pages.DebtDetailPage.performAction(map.get("action").toString());
