...
public class DebtBondRatingTest {
    	...
    	static String testDataName="debt_bond_rating";
    	
	    @Factory (dataProvider="dataProviderMethod")
	    public DebtBondRatingTest(Map<Object, Object> map){
	    	this.map=map;
	    ...
	    public void testCase() throws InterruptedException, Exception{
	        ...
	        try{
	        	  pages.DebtPricingPage.testData=map;
					 ...
					 String sTranName=ReadTestData.readTestDataBySpecifiedColumnNameAndReturnSpecificCellValue("add_transaction", "testTransactionID", map.get("testTransactionID").toString(), "tranName");
					  ...
					  if (sActivity.equalsIgnoreCase(OR.DP_TranName_BondIssue))
						  driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/deals/"+sObjectID+"/ratings");
							 ...
							 driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/others/"+sObjectID+"/ratings");
					  
					  pages.ActivityDashboardPage.WaitForPage(pages.DebtPricingPage);
...
//					  pages.DebtParticipantsPage.enterDebtParticipants_BondIssue(map,objSubTran);
					  ResultLog.ReportTestEvent("INFO", "Participant URL: "+TestDataRec.TESTDATALOGINMAP.get("url")+"/deals/"+sObjectID+"/ratings");
					  map.put("Activity", sActivity);
					  map.put("TranName", sTranName);
					  pages.DebtRatingPage.enterDebtRating_BondIssue(map,recordsubsetTestData);
					  if (sActivity.equalsIgnoreCase(OR.DP_TranName_BondIssue))
						  pages.DebtRatingPage.clickButton(OR.DebtRatingPage_DealCEPRatingsLbl);
					  else if (sTranName.equalsIgnoreCase("other activity"))
						  pages.DebtRatingPage.clickButton("Other CEP Ratings");
					  pages.DebtRatingPage.enterDebtCEPRating_BondIssue(map,recordsubsetTestData);
					  
					  pages.DebtRatingPage.verifyDataEntered(map);
...
//					  pages.CommonBusinessComponentPage.WaitForPage(pages.DebtPricingPage);
					  ResultLog.ReportTestEvent("INFO", "Screenshot before performing action on transaction - Update DEBT");
					  ResultLog.takeScreenShot();
