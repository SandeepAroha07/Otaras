package com.otaras.testautomation.testscripts.apptest.ford.functionaltest.newtran.task;
...
public class TaskAddTest {
    	...
    	static String testDataName="task";
    	
	    @Factory (dataProvider="dataProviderMethod")
	    public TaskAddTest(Map<Object, Object> map){
	    	this.map=map;
	    ...
	    public void testCase() throws InterruptedException, Exception{
	        ...
	        try{
				  ...
				  ResultLog.ReportTestEvent("INFO", "LOGIN TO APPLICATION AND NAVIGATE TO ADD DEBT PAGE"); 
					  ...
					  pages.MenuPage.clickTaskSubMenu();
					  pages.MenuPage.WaitForPage(pages.TaskPage);
...
//					  pages.MenuPage.WaitForPage(pages.AddClientPage);
				  ...
				  String tranKeyValue=pages.TaskPage.enterTaskData(map);
				  pages.TaskPage.uploadDocument();
...
//					  pages.AddClientPage.clickButton("Business Address");
//					  
//					  
...
//					  
				  ...
				  ResultLog.ReportTestEvent("INFO", "ADD DEBT PAGE - PERFORM TEST ACTION - "+map.get("action").toString());
					  ...
					  //working code - pages.TaskPage.verifySpecifiedElementExistsByLabel(map.get("actionStatus").toString());
					  //ResultLog.log(Status.INFO, "Google Page opened"+ResultLog.addScreenCaptureFromPath(captureScreen()));
					  pages.TaskPage.WaitForPage(pages.MasterEntityClientsPropsectsPage);
				  ...
				  ResultLog.ReportTestEvent("INFO", "##### VERIFY TRANSACTION DATA (USER DATA ENTERED IN UI) WITH DATABASE VALUES #####");
					  ...
					  String tbl_name="mvtasks";
		        	  String col_name ="prevLeadAdvisor" ;//"dealIssueTranName";
