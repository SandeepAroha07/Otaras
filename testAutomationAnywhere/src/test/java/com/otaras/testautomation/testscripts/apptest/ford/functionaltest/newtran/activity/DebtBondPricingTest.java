...
public class DebtBondPricingTest {
    	...
    	static String testDataName="debt_bond_pricing";
    	
	    @Factory (dataProvider="dataProviderMethod")
	    public DebtBondPricingTest(Map<Object, Object> map){
	    	this.map=map;
				  ...
				  pages.CommonBusinessComponentPage.LoginToApplication();
					 ...
					 String sTranName=ReadTestData.readTestDataBySpecifiedColumnNameAndReturnSpecificCellValue("add_transaction", "testTransactionID", map.get("testTransactionID").toString(), "tranName");
					  ...
					  if (sActivity.equalsIgnoreCase(OR.DP_TranName_BondIssue))
						  driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/deals/"+sObjectID+"/pricing");
					  else if (sTranName.equalsIgnoreCase("other activity"))
							 driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/others/"+sObjectID+"/pricing");
...
//					  pages.DebtParticipantsPage.enterDebtParticipants_BondIssue(map,objSubTran);
					  ResultLog.ReportTestEvent("INFO", "Participant URL: "+TestDataRec.TESTDATALOGINMAP.get("url")+"/deals/"+sObjectID+"/pricing");
					  pages.DebtPricingPage.enterDebtPricing_BondIssue(map,recordsubsetTestData);
					  
					  pages.DebtPricingPage.verifyDataEntered(map);
