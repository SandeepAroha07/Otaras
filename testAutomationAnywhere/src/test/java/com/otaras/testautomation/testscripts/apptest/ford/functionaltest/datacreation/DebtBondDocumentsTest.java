package com.otaras.testautomation.testscripts.apptest.ford.functionaltest.datacreation;
...
public class DebtBondDocumentsTest {
    	...
    	static String testDataName="debt_bond_documentsData";
    	
	    @Factory (dataProvider="dataProviderMethod")
	    public DebtBondDocumentsTest(Map<Object, Object> map){
	    	this.map=map;
	    ...
	    public void testCase() throws InterruptedException, Exception{
	        ...
	        try{
				...
				//map.put("activity","Bond Issue");
				//map.put("issuer","Florida State");	
				map.put("primarySector","Other");
				map.put("secondarySector","Irrigation");	
				//map.put("leadAdvisor","Isac Leland");	
				map.put("issueName","DataCreation");
				map.put("projectDescription","ProjectDescription");
				map.put("relatedTransactionType","");	
				map.put("relatedTransactionsIds","");
				map.put("notes","AutoTestTestDataCreation");
				//map1.put("action","Save");
	        	
	        	 pages.AddDebtPage.testData=map;   
...
//				  pages.MainPage.clickLoginIcon();
			  ...
			  ResultLog.ReportTestEvent("INFO", "LOGIN TO APPLICATION AND NAVIGATE TO ADD DEBT PAGE"); 
				  ...
				  //pages.LoginPage.verifyAppLoading();
			  ...
			  ResultLog.ReportTestEvent("INFO", "ADD DEBT PAGE - PERFORM TEST ACTION - "+map.get("action").toString());
				 ...
				 // Thread.sleep(2000);
	        	  ...
	        	  if (map.get("activity").toString().equalsIgnoreCase("bank loan")){
	        		  	tbl_name="tranbankloans";
	        	  		col_name = "actTranIssueName";}
	        	  else if (map.get("activity").toString().equalsIgnoreCase("bond issue")){
	        		  tbl_name="tranagencydeals";
	        	  		col_name ="dealIssueTranIssueName" ;}//"dealIssueTranName";}
	        	  ...
	        	  ResultLog.ReportTestEvent("PASS", "<< MongoDB Object ID >>"+documentID);
	        	
	        	
	        	
	        	
	        	
	        	pages.DebtDocumentsPage.testData=map;
...
//					  pages.LoginPage.WaitForPage(pages.HomePage);
					  ...
					  String sObjectID=documentID;//ReadTestData.readTestDataBySpecifiedColumnNameAndReturnSpecificCellValue("debt_bond_add", "testTransactionID", map.get("testTransactionID").toString(), "documentId");
					  
					  //http://mvqa1.us-east-1.elasticbeanstalk.com/deals/5b7b5891603b4034c7ef07de/details
					  //driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/deals/"+map.get("documentId").toString()+"/details");
					  if (map.get("activity").toString().equalsIgnoreCase("bank loan"))
						  driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/loan/"+sObjectID+"/documents");
					  else if (map.get("activity").toString().equalsIgnoreCase("bond issue"))
						  driver.get(TestDataRec.TESTDATALOGINMAP.get("url")+"/deals/"+sObjectID+"/documents");
					  
					  pages.ActivityDashboardPage.WaitForPage(pages.DebtDocumentsPage);
					  
					  ResultLog.ReportTestEvent("INFO", "DEBT - BONDS - DETAILS UPDATE"); 
...
//					int randomInt = min + ran.nextInt(max - min + 1);
					  ...
					  int maxAttachments=1;  //Default = 1 >> INCREASE THIS NUMBER FOR MORE THAN ONE
					  int randomAttachments = minNumber + ran.nextInt(maxAttachments - minNumber + 1);
					  
					  for(i=0;i<randomAttachments;i++){
						  randomInt = i + ran.nextInt(15 - i + 1);
						  
						  if (map.get("activity").toString().equalsIgnoreCase("bond issue"))
							  pages.DebtDocumentsPage.enterDebtDocuments_BondIssue(map);
						  else if (map.get("activity").toString().equalsIgnoreCase("bank loan"))
							  pages.DebtDocumentsPage.enterDebtDocuments_BankLoanIssue(map);
						  
						  if (map.get("activity").toString().equalsIgnoreCase("bond issue"))
							  pages.DebtDocumentsPage.uploadDocumentByIndex(randomInt);
						  else if (map.get("activity").toString().equalsIgnoreCase("bank loan"))
							  pages.DebtDocumentsPage.uploadDocumentBankLoanVyIndex(randomInt);
...
//						 uploadElement.sendKeys(sFilename);
				  ...
				  ResultLog.ReportTestEvent("INFO", "DEBT BONDS DETAIL PAGE - PERFORM TEST ACTION - "+map.get("action").toString());
					  pages.DebtDocumentsPage.performAction(map.get("action").toString());
					  pages.DebtDocumentsPage.Exists(pages.DebtDocumentsPage.PickStatus);
					  
					  pages.DebtDocumentsPage.WaitForPage(pages.DebtDocumentsPage);
					 // pages.ComplianceChecksPage.clickButton("Save");
					  Date d=new Date();
					  System.out.println("Before: "+d);
//					  if (map.get("activity").toString().equalsIgnoreCase("bank loan"))
//						  pages.DebtDocumentsPage.verifySpecifiedElementExistsByLabel1("Bank Loan Documents has been updated");
//					  if (map.get("activity").toString().equalsIgnoreCase("bond issue"))
//						  pages.DebtDocumentsPage.verifySpecifiedElementExistsByLabel1("Deal Documents has been updated");
					  d=new Date();
					  System.out.println("After"+ d);
					  }
					 // pages.ComplianceChecksPage.WaitForPage(pages.DebtDetailPage);
				  ...
				  ResultLog.ReportTestEvent("INFO", "UPDATE DB FOR EXECUTION STATUS");
					 // DBTestResultUpdate.UpdateTransactionIDInDB(testDataName,sTestID, "dealId",documentTranId);
					  ...
					  System.out.println("##################TEST STATUS: ################## "+ testReport.getStatus().toString().toUpperCase()+" "+sObjectID);
					  ResultLog.ReportTestEvent("PASS", "##################TEST STATUS: ################## "+ testReport.getStatus().toString().toUpperCase()+" "+sObjectID);
