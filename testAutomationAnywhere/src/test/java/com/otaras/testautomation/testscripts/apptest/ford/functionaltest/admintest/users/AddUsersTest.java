package com.otaras.testautomation.testscripts.apptest.ford.functionaltest.admintest.users;
...
public class AddUsersTest {
    	...
    	static String testDataName="adminNewUsers";
    	
	    @Factory (dataProvider="dataProviderMethod")
	    public AddUsersTest(Map<Object, Object> map){
	    	this.map=map;
	    ...
	    public void testCase() throws InterruptedException, Exception{
	        ...
	        try{
	        	  pages.AdminAddUsersPage.testData=map;                 
				  ...
				  ResultLog.ReportTestEvent("INFO", "LOGIN TO APPLICATION AND NAVIGATE TO ADD DEBT PAGE"); 
					...
					//  pages.CommonBusinessComponentPage.sleep();
					  	pages.CommonBusinessComponentPage.SelectAdminMenu();
					  	//sandeep
					//  	pages.CommonBusinessComponentPage.sleep();
					    pages.CommonBusinessComponentPage.waitForuntilloading(OR.loader_image);
					  
					  pages.AdminFeaturesSelectionPage.Click(OR.AdminAddUsersPage_Users);
					//  pages.CommonBusinessComponentPage.sleep();
					//  pages.CommonBusinessComponentPage.sleep();
					 // pages.CommonBusinessComponentPage.sleep();
					  pages.CommonBusinessComponentPage.waitForuntilloading(OR.loader_image);
					  pages.AdminFeaturesSelectionPage.WaitForPage(pages.AdminUsersListPage);
					  //pages.AdminUsersListPage.clickElementByLabel("Add new contact");
					  pages.AdminUsersListPage.waitElementClickable(OR.AdminUsersListPage_AddNewContact);
					  
					  pages.AdminUsersListPage.Click(OR.AdminUsersListPage_AddNewContact);
					  pages.CommonBusinessComponentPage.waitForuntilloading(OR.loader_image);
					  pages.AdminUsersListPage.WaitForPage(pages.AdminAddUsersPage);
					  
					  String  tranKeyValue=pages.AdminAddUsersPage.enterNewUserClientData(map);
				//	  driver.findElement(By.xpath("//div[@class='control']/*[contains(text(),'"+"Save Other Details"+"')]/parent::div")).click();
			//		  pages.AdminUsersListPage.waitForElementExists("//div/*[contains(text(),'"+"Other Details"+"')]");
					  
					  ResultLog.ReportTestEvent("INFO", "Enter Address Data"); 
//					  pages.AdminAddUsersPage.clickButton("Address");
//					  pages.AdminAddUsersPage.enterMaster_AddressData("businessAddress", "key", map.get("address").toString());
//					  pages.AdminAddUsersPage.enterUserDocuments(map);
//					  pages.AdminAddUsersPage.uploadDocument();
					  
//					  pages.MenuPage.clickActivitySubMenu();
//					  pages.MenuPage.WaitForPage(pages.NewActivitySelectionPage);
//					  
//					  pages.NewActivitySelectionPage.clickElementByLabel("Derivative");
//					  pages.MenuPage.WaitForPage(pages.AddDerivativePage);
				
				  ResultLog.ReportTestEvent("INFO", "ADD DEBT"); 
					  //pages.AddDerivativePage.enterDebtDetail_Derivative(map);
...
//					  pages.AdminAddUsersPage.WaitForPage(pages.AdminUsersListPage);
//					  //ResultLog.takeScreenShot();
//					  
//					  pages.AdminUsersListPage.Exists(pages.AdminUsersListPage.SearchUsers);
//					  pages.AdminUsersListPage.SearchUsers.sendKeys(tranKeyValue);
//					  //Thread.sleep(1000);
//					  pages.AdminUsersListPage.waitForElementExists("//div/*[contains(text(),'"+tranKeyValue+"')]");
//					  //pages.ProjectsDashboardPage.Exists(driver.findElement(By.xpath("//a/mark[contains(text(),'"+tranKeyValue+"')]")));
//					  pages.ProjectsDashboardPage.clickElement(driver.findElement(By.xpath("//a/*[contains(text(),'"+map.get("firstName").toString()+"')]")));
//					  //driver.findElement(By.xpath("//a/mark[contains(text(),'"+tranKeyValue+"')]")).click();
//					  pages.ProjectsDashboardPage.WaitForPage(pages.AdminAddUsersPage);
		...
		//working code-anil commented-defectlogged			  pages.AddDebtPage.verifySpecifiedElementExistsByLabel(map.get("actionStatus").toString());
				  ...
				  ResultLog.ReportTestEvent("INFO", "##### VERIFY TRANSACTION DATA (USER DATA ENTERED IN UI) WITH DATABASE VALUES #####");
					  ...
					  String tbl_name="entityusers";
		        	  String col_name ="userLastName";//"userEmployeeID" ;//"dealIssueTranName";
		        	  //String  srch_string = "TXDEALE80FA6B0A36E11E8";
		        	  //String  tranKeyValue = pages.AddDerivativePage.tranIssueNameKeyValue();
...
//		        	  Document TrandocumentObj= mongodbutil.getDocumentObject("entityusers","_id", dealIssueTranAssignedTo);
				  ...
				  ResultLog.ReportTestEvent("INFO", "UPDATE DB FOR EXECUTION STATUS");
					  ...
					  System.out.println("##################TEST STATUS: ################## "+ testReport.getStatus().toString().toUpperCase()+"   Document ID: "+documentID);
					  ResultLog.ReportTestEvent("PASS", "##################TEST STATUS: ################## "+ testReport.getStatus().toString().toUpperCase()+"   Document ID: "+documentID);
